...
DATA_DIR = "./"#was ../data/ ,Ammar
TRAINING_SET_SIZE = 9367 #was 3380, Ammar
BATCH_SIZE = 10
IMAGE_SIZE = 224
...
def flower_input(if_random = True, if_training = True):
    if(if_training):
        filenames = [os.path.join(DATA_DIR, "train-0000%d-of-00002.tfrecord" % i) for i in xrange(0, 1)] #change from folwer_train... ,Ammar
    else:
        filenames = [os.path.join(DATA_DIR, "train-0000%d-of-00002.tfrecord" % i) for i in xrange(0, 1)] #changed from flower_eval..., Ammar
...
#    loss = tf.reduce_sum(tf.nn.softmax_cross_entropy_with_logits(labels=label_batch_one_hot, logits=logits_out))
    ...
    with tf.Session() as sess:
        ...
        #saver.restore(sess, "/home/ammarnahari/ros_ws/src/484_project/training/output/checkpoint-train.ckpt")
        ...
        for i in range(TRAINING_SET_SIZE * 100):
            ...
            print(loss_out)
            if(i%50 == 0):
                saver.save(sess, "/home/ammarnahari/ros_ws/src/484_project/training/output/checkpoint-train.ckpt")
